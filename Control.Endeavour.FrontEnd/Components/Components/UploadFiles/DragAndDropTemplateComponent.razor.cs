...
namespace Control.Endeavour.FrontEnd.Components.Components.UploadFiles
{
    public partial class DragAndDropTemplateComponent : ComponentBase
    {
        ...
        {
            if (TypeOfFiles == TypeOfFilesEnum.ProfilePicture || TypeOfFiles == TypeOfFilesEnum.Logo || TypeOfFiles == TypeOfFilesEnum.Banner)
            {
                containerClass = "";
            }
        }
        ...
        private async Task HandleInvalidFile(IBrowserFile file)
        {
            if (!IsFileCountValid())
            {
                notificationModal.UpdateModal(ModalType.Error, "¡Has superado la cantidad de archivos permitidos!", true, "Aceptar");
            }
            else
            {
